...
from batch.compare_travel.keywords import COMPARE_TRAVEL_KEYWORDS
from batch.compare_travel.select_column import SOURCES_SELECT_MAP
from batch.fetch import fetch_data
from batch.utils import read_csv
from batch.variables import COMPARE_TRAVEL_DATA_PATH, SAVE_PATH, SOURCES
from logger import logger


def collect_load_compare_travel_data(queries: list[str]) -> None:
    """데이터를 수집하고 저장."""
    os.makedirs(SAVE_PATH, exist_ok=True)

    _df_list: list[pd.DataFrame] = [read_csv(COMPARE_TRAVEL_DATA_PATH)]
    for source in tqdm(SOURCES, disable=True):
        logger.info(f"{source} scrap started")
        _file_path = os.path.join(SAVE_PATH, f"_{source}_compare_travel.csv")
    ...
    data.to_csv(COMPARE_TRAVEL_DATA_PATH, index=False, encoding="utf-8")
